...
import { createReadStream, statSync, mkdirSync, readFileSync } from 'node:fs'
...
export default {
  ...
  apply(ctx, cfg) {
    ...
    // Steam can span multiple library folders; the registry key only points at
    // the primary one. Parse libraryfolders.vdf to cover secondary libraries.
    function detectSteamRoot() {
      try {
        ...
        if (m && m[1]) {
          const primary = m[1].trim().replace(/[\\/]+$/, '')
          const candidates = [primary]
          try {
            const vdf = readFileSync(primary + '\\steamapps\\libraryfolders.vdf', 'utf8')
            const re = /"path"\s+"([^"]+)"/g
            let mm
            while ((mm = re.exec(vdf))) {
              const p = mm[1].replace(/\\\\/g, '\\').trim()
              if (p) candidates.push(p.replace(/[\\/]+$/, ''))
            }
          } catch (e) {}
          for (const c of candidates) {
            const root = c + '\\' + WORKSHOP_SUFFIX
            if (statSync(root, { throwIfNoEntry: false })) return root
          }
        }
      } catch (e) {}
      return null
